...
import { ArrowLeft, CalendarDays, CheckCircle2, Clock3, IndianRupee, MapPin, MessageSquareText, Tv, UserRound, XCircle } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import Sidebar from '../../components/Sidebar';
import { Button } from '../../components/ui/button';
import { toast } from 'sonner';
import { defaultMediaRequests, MEDIA_VENDOR_REQUESTS_STORAGE_KEY } from './mediaRequestData';
...
const MediaRequests = () => {
  ...
  const [requests, setRequests] = useState(() => {
    if (typeof window === 'undefined') {
      return defaultMediaRequests;
    }

    const stored = window.localStorage.getItem(MEDIA_VENDOR_REQUESTS_STORAGE_KEY);
    return stored ? JSON.parse(stored) : defaultMediaRequests;
  });

  useEffect(() => {
    if (typeof window !== 'undefined') {
      window.localStorage.setItem(MEDIA_VENDOR_REQUESTS_STORAGE_KEY, JSON.stringify(requests));
    }
  }, [requests]);
  ...
  const updateRequestStatus = (requestId, status) => {
    ...
    toast.success(status === 'accepted' ? 'Media request accepted' : 'Media request rejected');
  };
      ...
      <main className="flex-1 p-8 lg:p-10">
        <div className="mx-auto max-w-6xl space-y-8">
          <section className="rounded-[32px] bg-[linear-gradient(135deg,#0b1437_0%,#1d3fbf_60%,#8cb4ff_100%)] p-8 lg:p-10 text-white shadow-xl">
            <button
              type="button"
              onClick={() => navigate('/vendor/dashboard/media')}
            ...
            <div className="mt-5 max-w-3xl">
              ...
              <h1 className="mt-4 text-4xl font-heading font-bold">Review pending media requests</h1>
              <p className="mt-3 text-base font-body text-white/80">
                Check placement details, campaign timing, and briefing notes before accepting or rejecting a producer request.
              </p>
            </div>
          </section>
          ...
          <section className="grid gap-6 md:grid-cols-3">
            <article className="rounded-[28px] border border-[#dbe3f3] bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 text-sm text-[#667085]">Requests waiting for media placement confirmation.</p>
            </article>
            <article className="rounded-[28px] border border-[#dbe3f3] bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 text-sm text-[#667085]">Accepted media requests are ready for producer payment.</p>
            </article>
            <article className="rounded-[28px] border border-[#dbe3f3] bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 text-sm text-[#667085]">Rejected requests remain available for vendor-side tracking.</p>
            </article>
          </section>
                  ...
                  <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="mt-2 text-sm text-[#667085]">Requested by {request.producerName}</p>
                      </div>
                      ...
                      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                        ...
                        <div className="rounded-2xl bg-[#f8faff] p-4">
                          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#8a94a6]">Brief</p>
                      ...
                      {isPending ? (
                        ...
                        >
                          {isAccepted
                            ...
                            : 'Request rejected. This media placement remains unavailable for this campaign.'}
                        </div>
                      )}
                    </div>
                  </div>
